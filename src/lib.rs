...
macro_rules! gen {
    ($fn_name: ident($($arg: ident: $arg_ty: ty),*) -> $ret_ty: ty, { $($ref_arg: ident),*}) => {
        #[no_mangle]
        pub unsafe extern "C" fn $fn_name($($arg: $arg_ty),*) -> $ret_ty {
            use std::io::Write;
            use std::io::stdout;
            let args : &[String] = &[$(
                format!(".{}={:x?}", stringify!($arg), $arg),
            )*];
            print!("{}({}) = ", stringify!($fn_name), args.join(", "));
            stdout().flush().unwrap();

            let sym = $crate::CUDADRV_SYMBOLS.get_symbol(stringify!($fn_name));
            let fn_ptr: extern "C" fn($($arg_ty),*) -> $ret_ty = ::core::mem::transmute(sym);
            let res = fn_ptr($($arg),*);

            println!("{:?}", res);

            $(
                println!("  > *.{} = {:x?}", stringify!($ref_arg), unsafe{*$ref_arg});
            )*
...
macro_rules! cuda_fn {
    ...
    ($fn_name: ident($($arg: ident: $arg_ty: ty),* $(,)?) -> $ret_ty: ty, { $($ref_arg: ident),* $(,)? }) => {
        gen!($fn_name($($arg: $arg_ty),*) -> $ret_ty, {$($ref_arg),*});
    }

}

cuda_fn!(cuInit(flags: c_uint) -> CUresult);
cuda_fn!(cuGetErrorString(error: CUresult, str_ref: *mut *const c_char) -> CUresult);
cuda_fn!(cuGetErrorName(error: CUresult, str_ref: *mut *const c_char) -> CUresult);
...
cuda_fn!(cuModuleLoad(cmod_ref: *mut CUmodule, file_name: *const c_char) -> CUresult, {cmod_ref});
cuda_fn!(cuModuleGetFunction(func_ref: *mut CUfunction, cmod: CUmodule, func_name: *const c_char) -> CUresult, {func_ref});
cuda_fn!(cuModuleGetGlobal(dptr_ref: *mut CUdeviceptr, bytes_ref: *mut usize, module: CUmodule, name: *const c_char) -> CUresult, {dptr_ref, bytes_ref});
cuda_fn!(cuModuleGetGlobal_v2(dptr_ref: *mut CUdeviceptr, bytes_ref: *mut usize, module: CUmodule, name: *const c_char) -> CUresult, {dptr_ref, bytes_ref});
